...
(function () {
        ...
        enrichmentState.persistTimer = window.setTimeout(() => {
            ...
            try {
                window.localStorage.setItem(
                    CONFIG.orderCacheStorageKey,
                    JSON.stringify(enrichmentState.cache)
                );
            } catch (error) {
            ...
            if (candidate && typeof candidate === 'object') {
                ...
                return {
                    ...
                    productId: normalizeProductId(
                        candidate.productId || candidate.itemId || candidate.auctionId || '',
                        orderId
                    ),
                    productTitle: normalizeText(candidate.productTitle || candidate.title || candidate.itemTitle || ''),
                    productPrice: normalizePriceText(
                        candidate.productPrice || candidate.price || candidate.actualFee || candidate.payAmount || ''
                    ),
                    purchaseQuantity: normalizeQuantityText(
                        candidate.purchaseQuantity || candidate.quantity || candidate.num || candidate.buyAmount || ''
                    ),
    ...
    function readOrderIdFromCard(cardEl) {
        const headerTexts = Array.from(
            cardEl.querySelectorAll('.header-wrapper .hearder-left, .header-wrapper span')
        )
    ...
    function isReceiverInfoComplete(info = {}) {
        return !!(
            normalizeText(info.receiverAddress)
    ...
    function buildBaseOrderFromCard(cardEl) {
        ...
        const orderId = pickPreferredField(
            readOrderIdFromCard(cardEl),
            extractOrderIdFromLines(lines),
            links.params.orderId,
            links.params.bizOrderId,
            links.params.tradeId,
            dataset.orderId,
            reactSummary.orderId
        );
        ...
        const order = {
            orderId,
            buyerName: pickPreferredField(
                cached.buyerName,
                wwNode?.dataset?.tnick,
                wwNode?.dataset?.nick,
                reactSummary.buyerName
            ),
            buyerUserId: pickPreferredField(
                cached.buyerUserId,
                wwNode?.dataset?.encryptuid,
                dataset.encryptuid,
                reactSummary.buyerUserId
            ),
            productId: pickPreferredField(
                extractProductIdFromLines(lines, orderId),
                extractProductIdFromLinks(links, orderId),
                normalizeProductId(dataset.productId, orderId),
                normalizeProductId(dataset.itemId, orderId),
                normalizeProductId(dataset.auctionId, orderId),
                normalizeProductId(reactSummary.productId, orderId),
                normalizeProductId(cached.productId, orderId)
            ),
            productTitle: pickPreferredField(
                cached.productTitle,
                normalizeText(cardEl.querySelector('a[href*="tradeSnap.htm"] div')?.textContent),
                normalizeText(cardEl.querySelector('a[href*="tradeSnap.htm"]')?.textContent),
                dataset.productTitle,
                reactSummary.productTitle
            ),
            productPrice: pickPreferredField(
                normalizePriceText(dataset.productPrice),
                priceQuantity.productPrice,
                reactSummary.productPrice
            ),
            purchaseQuantity: pickPreferredField(
                normalizeQuantityText(dataset.purchaseQuantity),
                priceQuantity.purchaseQuantity,
                reactSummary.purchaseQuantity
            ),
            receiverName: pickPreferredField(
                cached.receiverName,
                receiverInfo.receiverName,
                reactSummary.receiverName
            ),
            receiverPhone: pickPreferredField(
                cached.receiverPhone,
                receiverInfo.receiverPhone,
                reactSummary.receiverPhone
            ),
            receiverAddress: pickPreferredField(
                cached.receiverAddress,
                receiverInfo.receiverAddress,
                reactSummary.receiverAddress
            ),
            orderStatusText: pickPreferredField(
                lines.find(line => /已支付|待发货|待处理|待揽收|待配送/.test(line)),
                dataset.orderStatusText,
                reactSummary.orderStatusText
            ),
            paidAtText: pickPreferredField(
                extractLabeledValueFromCard(cardEl, '支付时间', /^\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}(?::\d{2})?$/),
                extractLabeledTime(lines, '支付时间'),
                dataset.paidAt,
                reactSummary.paidAtText
            ),
            latestShipAtText: pickPreferredField(
                extractLabeledValueFromCard(cardEl, '最晚发货时间', /^\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}(?::\d{2})?$/),
                extractLabeledTime(lines, '最晚发货时间'),
                dataset.latestShipAt,
                reactSummary.latestShipAtText
            ),
            raw: {
                textLines: lines,
                hrefs: links.hrefs,
    ...
    function extractProductIdFromTradeSnapshotHtml(html, orderId = null) {
        ...
        for (const row of keyValueRows) {
            ...
            const valueText = normalizeText(
                row.querySelector('.value-inline')?.textContent
        ...
        const sourceMatch = htmlText.match(
            /(?:itemId|auctionId|item_id|auction_id)["'=:\s>]+["']?(\d{6,20})/i
        );
        if (sourceMatch) {
    ...
    function findNextPageButton() {
        return Array.from(document.querySelectorAll('button, a, li, span'))
            .find(el => {
                ...
                if (!/下一页|下页/.test(text)) return false;

...
function stripQuotedReplyPrefix(
    ...
    if (!normalizedContent || type !== 'text' || !repliedMessage) {
        return normalizedContent;
    }

    if (repliedMessage.type === 'image') {
        const withoutAliasLine = stripLeadingEquivalentAuthorLine(normalizedContent, repliedAuthorLabel);
        return withoutAliasLine || normalizedContent;
    }
    ...
    const strippedReplyHeader = stripLeadingReplyHeader(normalizedContent, quotedText, repliedAuthorLabel);
    if (strippedReplyHeader !== normalizedContent) {
    ...
    const strippedAuthorThenQuote = stripAuthorThenQuotedTextBlock(normalizedContent, quotedText, repliedAuthorLabel);
    if (strippedAuthorThenQuote !== normalizedContent) {
        return strippedAuthorThenQuote || normalizedContent;
    }

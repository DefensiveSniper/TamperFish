...
const { cacheRemoteImages, localizeMessages, localizeSessions, serveCachedMediaRequest, } = require('./media_cache');
const app = express();
...
app.get('/api/sessions/:chatKey/messages', async (req, res) => {
    try {
        ...
        const messages = await localizeMessages(await db.getMessages(req.params.chatKey), { publicOrigin: BROWSER_MEDIA_ORIGIN });
...
app.post('/api/outgoing-messages', async (req, res) => {
    const { chatKey, sessionId, content, messageType = 'text', mediaData = null, mediaName = null, replyToExternalMessageId = null, replyToPreview = null, replyToType = null, source = 'manual', customerName = null, productId = null, } = req.body || {};
    ...
    try {
        ...
        const result = await db.addOutgoingMessage(effectiveChatKey, {
...
function ensureBrowserWssTlsMaterial() {
    ...
    if (!certExists || !keyExists) {
        ...
        try {
            ...
            throw new Error(`无法生成 localhost WSS 证书：${error.message}。`
                + ' 请安装 openssl，或通过 BROWSER_WSS_CERT_PATH / BROWSER_WSS_KEY_PATH 提供证书。');
        }
        if (result.status !== 0) {
            throw new Error(`openssl 生成 localhost WSS 证书失败：${(result.stderr || result.stdout || '').trim() || 'unknown error'}`);
        }
        generated = true;
    }
    return {
...
async function handleBrowserRpcAction(action, payload = {}) {
    switch (action) {
        ...
        case 'orders.heartbeat': {
            const { pageUrl = '', visibleOrderCount = 0, scanState = 'idle', scanNonceHandled = null, syncNonceHandled = null, } = payload || {};

...
function ensureChromeProxyExtension(config) {
    ...
    const normalizedProxy = normalizeChromeProxyServer(config.chromeProxyServer, config.chromeProxyScheme);
    if (!normalizedProxy) {
        return null;
    }
...
function selectCanonicalFilteredEventValue(entries, preferredKey) {
    ...
    const populatedEntry = entries.find((entry) => Array.isArray(entry.value) && entry.value.length > 0);
    if (populatedEntry) {
...
function clearSecurePreferenceProtectionForExtension(securePreferences, extensionId) {
    ...
    if (extensionProtection?.settings_encrypted_hash
...
function repairTampermonkeyWebRequestExplosion(config, profile) {
    ...
    const securePreferencesPath = path.join(config.chromeUserDataDir, profile.profileDirectory, 'Secure Preferences');
    ...
    for (const [extensionId, extensionSetting] of Object.entries(extensionSettings)) {
        ...
        const result = compactFilteredWebRequestEvents(extensionSetting, config.tampermonkeyWebRequestEventThreshold);
        if (!result.changed) {
            continue;
        }
    ...
    for (const summary of repairSummaries) {
        log(`启动前修复 ${summary.name} (${summary.extensionId}) 的 webRequest 子事件膨胀: `
...
async function launchChromeAttempt(config) {
    ...
    log(`拉起 Chrome profile "${profile.displayName}" (${profile.profileDirectory})，用户数据目录 ${config.chromeUserDataDir}，监听 ${config.cdpPort}`);
...
async function ensureChromeDebugging(config) {
    ...
    terminateChromeChild(launchAttempt.child, `Chrome 启动超时，${config.chromeStartTimeoutMs}ms 内未开放 ${config.cdpPort}，准备结束当前实例`);
...
function startChromeWatchdog(config) {
    ...
    chromeWatchTimer = setInterval(async () => {
        ...
        chromeEnsuring = true;
        try {
            log(`检测到 Chrome 调试端口 ${config.cdpPort} 已关闭，准备使用 ${config.chromeUserDataDir} 自动恢复浏览器`);
...
async function main() {
    ...
    const chromeProxyConfigPath = process.env.CHROME_PROXY_CONFIG_PATH || DEFAULT_CHROME_PROXY_CONFIG_PATH;
    ...
    const config = {
        ...
        chromeMonitorIntervalMs: Number(process.env.CHROME_MONITOR_INTERVAL_MS || DEFAULT_CHROME_MONITOR_INTERVAL_MS),
        ...
        chromeProxyDisabled,
        chromeProxyServer: chromeProxyDisabled ? '' : process.env.CHROME_PROXY_SERVER || localChromeProxyConfig.proxyServer || '',
        chromeProxyScheme: process.env.CHROME_PROXY_SCHEME ||
            localChromeProxyConfig.proxyScheme ||
            DEFAULT_CHROME_PROXY_SCHEME,
        chromeProxyUsername: chromeProxyDisabled ? '' : process.env.CHROME_PROXY_USERNAME || localChromeProxyConfig.proxyUsername || '',
        chromeProxyPassword: chromeProxyDisabled ? '' : process.env.CHROME_PROXY_PASSWORD || localChromeProxyConfig.proxyPassword || '',
        chromeProxyBypassList: process.env.CHROME_PROXY_BYPASS_LIST ||
        ...
        chromeClearTransientDataOnStart: chromeClearTransientDataEnv === '0'
            ? false
        ...
        chromeRepairTampermonkeyWebRequestOnStart: process.env.CHROME_REPAIR_TAMPERMONKEY_WEBREQUEST_ON_START === '0'
            ? false
            : DEFAULT_CHROME_REPAIR_TAMPERMONKEY_WEBREQUEST_ON_START,
        tampermonkeyWebRequestEventThreshold: Number.isFinite(rawTampermonkeyThreshold) && rawTampermonkeyThreshold > 0
